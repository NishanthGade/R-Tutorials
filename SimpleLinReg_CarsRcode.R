...
regr.eval(train$Price, as.vector(LinReg1$fitted.values),train.y=train$Price) 
#Error verification on test data
Pred<-predict(LinReg1,test)
regr.eval(test$Price, Pred)
a<-data.frame(test$Price,Pred)

plot(test$Age_06_15, test$Price, 
     ylab="Price", xlab="Age_06_15", 
     main="Regression Price plot") 
